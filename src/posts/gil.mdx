...
A program under execution is called a process. It can have multiple threads.
...
Python Interpreter is like a Virtual Machine. The thread is basically a POSIX or OS thread.
...
The check for 100 ticks is validated for Python 2, it is removed with the advent of Python 3. As there are many cons here (see in graph)
...
In Python 3.13 release (October 8, 2024) they have attempted experimental **GIL-free mode** to remove this limitation. It will allow threads to execute Python code concurrently on multiple cores, improving the performance of multi-threaded applications.

...
it may look easier as we get started but causes a lot of problems later on.

as the application gets bigger it becomes harder to keep track of all variables and what type they should be.
...
what pydantic is?
in simpler terms, it is data validation library in python. You may find using pydantic in top python libraries like huggingface/transformer, fastapi, langchain, airflow etc.
...
# if you've json string and want to convert it back into pydantic model
...
new_product1 = myProduct(**new_product1_dict)
```

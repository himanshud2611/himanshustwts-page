...
I've been trying to get into rabbit holes of diffusion generative modelling for quite some time. I started drafting this blog few months back but didn't get time to finish it up. This is some raw effort to understand KLD in diffusion generative models from ground up and beyond.
...
![image.png](/dgm-kl-divergence/image.png)
...
**Reverse KL**, $D_{\text{KL}}(p_{\phi} \,\|\, p_{data})$, weights errors by your model's distribution. It permits ignoring data modes entirely but harshly penalizes hallucination. This induces **mode seeking.** Your model learns to concentrate on a subset of the data it can explain well, sacrificing coverage for precision.
...
PS: Check out [Ground Zero](https://x.com/groundzero_twt) for deep tech podcasts with interesting folks in AI and more (coming soon).

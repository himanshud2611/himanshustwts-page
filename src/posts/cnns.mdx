...
Feeling overwhelmed while looking at the content above? Don't worry, just go through the article and leave the rest on me.
...
This is a very fundamental operation in convolution. We perform the same operation between input & kernel that will further produce the output (another matrix).
...
Take each matrix in first kernel and compute cross-correlation with the input data. Sum three results and add up the first bias. This will produce the first output.
...
Let’s start with an example, computing the derivative of E w.r.t B1 (using helper equation -2)
...
Surprised, how did we get this. Look over the forward propagation equation, we’ve just found the derivative of all equation w.r.t Xj
...
Next, we compute the input gradient. We specifically have chosen convolve2d method and mentioned mode to be “full”, which represent the full convolution.
...
Our goal is to solve the MNIST dataset.
...
The reshape layer like all the other layers inherits from the base layer class, its constructor takes in the shape of input and the shape of output. Forward method reshapes the input to the output.
